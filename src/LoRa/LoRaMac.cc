...
void LoRaMac::initialize(int stage) {
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        txQueue = check_and_cast<queueing::IPacketQueue*>(
                getSubmodule("queue"));
...
void LoRaMac::handleSelfMessage(cMessage *msg) {
    ...
    if (msg == endPingSlot) {
        ...
        scheduleAt(
                simTime() + (pingOffset * slotLen) + timeToNextSlot - slotLen,
                pingPeriod);
        EV << "scheduling Next Ping Slot at "
                  << simTime() + (pingOffset * slotLen) + timeToNextSlot
                          - slotLen << endl;
        scheduleAt(simTime() + (pingOffset * slotLen) + timeToNextSlot,
                endPingSlot);
    }
    handleWithFsm(msg);
}
...
//    LoRaMacControlInfo *cInfo = check_and_cast<LoRaMacControlInfo *>(msg->getControlInfo());
    ...
    if (fsm.getState() != IDLE)
        EV << "deferring upper message transmission in " << fsm.getStateName()
                  << " state\n";
    else {
        popTxQueue();
...
void LoRaMac::handleLowerMessage(cMessage *msg) {
    if ((fsm.getState() == RECEIVING_1) || (fsm.getState() == RECEIVING_2)
        ...
        if (frame->getPktType() == BEACON) {
            ...
            EV << "RECEIVED A DOWNLINK MESSAGE WITH A DPAD OF : " << DPAD
                      << endl;
            std::cout << "RECEIVED A DOWNLINK MESSAGE WITH A DPAD OF : " << DPAD
                    << std::endl;
...
void LoRaMac::receiveSignal(cComponent *source, simsignal_t signalID,
...
Packet* LoRaMac::decapsulate(Packet *frame) {
    ...
    frame->addTagIfAbsent<MacAddressInd>()->setSrcAddress(
            loraHeader->getTransmitterAddress());
    frame->addTagIfAbsent<MacAddressInd>()->setDestAddress(
            loraHeader->getReceiverAddress());
    frame->addTagIfAbsent<InterfaceInd>()->setInterfaceId(
...
void LoRaMac::sendAckFrame() {
    ...
    macHeader->setReceiverAddress(
            MacAddress(
                    frameToAck->peekAtFront<LoRaMacFrame>()->getTransmitterAddress().getInt()));
...
void LoRaMac::calculatePingPeriod(const Ptr<const LoRaMacFrame> &frame) {
    ...
    cipher[0] = (unsigned char) (frame->getBeaconTimer()
            + getAddress().getAddressByte(0) + getAddress().getAddressByte(1)
            + getAddress().getAddressByte(2) + getAddress().getAddressByte(3)
            + getAddress().getAddressByte(4) + getAddress().getAddressByte(5));
...
int LoRaMac::aesEncrypt(unsigned char *message, int message_len,
...
void LoRaMac::beaconThings() {
    ...
    scheduleAt(simTime() + beacon_period - beacon_reserved - beaconGuard,
            beaconGuardPeriod);
...
void LoRaMac::finishCurrentTransmission() {
    ...
    scheduleAt(simTime() + waitDelay1Time + listening1Time + waitDelay2Time,
            endDelay_2);
    scheduleAt(
            simTime() + waitDelay1Time + listening1Time + waitDelay2Time
                    + listening2Time, endListening_2);
    deleteCurrentTxFrame();
}
